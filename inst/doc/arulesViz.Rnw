...
the unified interface provides by ~\pkg{arulesViz} for association
rule visualization.
...
\section{Data preparation and unified interface of \pkg{arulesViz}}
\label{sec:prep}
...
$$\set{R} = \{
    \langle a_1,c_1,m_1\rangle, 
    \langle a_2,c_2,m_2\rangle, \ldots 
    \langle a_n,c_n,m_n\rangle\}$$
where $a_i$ is the antecedent, $c_i$ is the consequent and $m_i$ is the 
selected interest measure
for the $i$-th rule.  In $\set{R}$ we identify the set of $K$ unique
...
$\mat{M}_{lk} = m_i$ for $i=\{1,2,\ldots,n\}$ and $l$ and $k$ corresponding
...
<<matrix_col1, fig=TRUE, height=3, width=10, results=hide, include=FALSE>>=
plot(subrules, method="matrix", measure=c("lift", "confidence"))
@
<<matrix_col2, fig=TRUE, height=3, width=10, results=hide, include=FALSE>>=
plot(subrules, method="matrix", measure=c("lift", "confidence"),
...
techniques that enhances matrix-based visualization using grouping of rules
...
distance functions lose their usefulness. The situation is even getting worse
...
itemsets, two subsets of a frequent itemset will both cover many common
...
rendered using the interface to the \emph{GraphViz} software in 
package \pkg{Rgraphviz}~\citep{arules:Gentry:2010}. 
The default
version represents itemsets as vertices and rules as directed edges
between itemsets (shown in Figure~\ref{fig:graph1}).
...
The ``bipartite'' version splits the set of itemsets
into two separate vertex sets, one for each antecedents and consequences
(shown in Figure~\ref{fig:graph2}).

<<graph2, fig=TRUE, height=10, include=FALSE>>=
plot(subrules2, method="graph",
...
\begin{figure}
\centering
\includegraphics[width=10cm]{arulesViz-graph2}
\caption{Graph-based visualization with two separate sets of itemsets (antecedent and consequent sets).
\label{fig:graph2}}
\end{figure}

The third variant uses items and rules as two types of vertices and
edges indicate which items occur in which rule.

<<graph3, fig=TRUE, width=10, height=10, include=FALSE>>=
plot(subrules2, method="graph",
...
The built-in graph based visualizations available in \pkg{arulesViz} are only
...
as a Graphviz dot-file~\citep{arules:Gentry:2010}
to be explored in tools like Gephi. For example the
1000 rules with the highest lift are exported by:

%\marginpar{this code needs to be adapted and we need to replace the figure with a screenshot. Probably we need to rewrite the export code, too!}
...
discovered classification rules \cite{arulesViz:cviz} 
and association rules \cite{arulesViz:Yang:2003}. \cite{arulesViz:Yang:2003} 
